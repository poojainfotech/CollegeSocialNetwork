...
namespace CollegeSocialNetwork.Quiz
{
    public partial class AddQuiz : System.Web.UI.Page
    {
        ...
        protected void btnUploadQuiz_Click(object sender, EventArgs e)
        {
            ...
            string UserName = "";
            string fileStatus = "Properformate";
            int recordCount = 0;
                ...
                try
                {
                    ...
                    if (type != String.Empty)
                    {
                        ...
                        while ((line = file.ReadLine()) != null)
                        {
                            ...
                            if (fields.Length != 9)
                            {
                                fileStatus = "NotInProperformate";
                            }                            
                        }

                        if(fileStatus == "Properformate")
                        {
                            if (recordCount > 50)
                            {
                                Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "recordInfile();", true);
                            }
                            else
                            {
                                dataTable = dataBaseUtility.GetDataSet("sp_TrucateQuizTable", "TrucateQuizTable", null);
                                ...
                                while ((line = file.ReadLine()) != null)
                                {
                                    ...
                                    if (fields.Length == 9)
                                    {
                                        ...
                                        paramName.Enqueue(new SqlParameter("@AcademicYear", fields[6].Split(':')[1].ToString()));
                                        paramName.Enqueue(new SqlParameter("@CourseName", fields[7].Split(':')[1].ToString()));
                                        paramName.Enqueue(new SqlParameter("@SubjectName", fields[8].Split(':')[1].ToString()));
                        ...
                        {
                            Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "fileFormate();", true);
                        }
